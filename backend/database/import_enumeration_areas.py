...
def _feature_payload(feature: dict[str, Any], index: int, source_dataset: str) -> tuple[str, str, str | None, str, str, str]:
    ...
    ea_id = _first_present(props, ["id", "ea_id", "eaid", "ea_code", "code", "objectid"], f"ea-{index + 1}")
    ...
    ea_name = _first_present(props, ["ea_name", "ea", "name", "enumeration_area", "enumeration_area_name"], ea_id)
...
def import_geojson(path: Path, source_dataset: str) -> int:
    ...
    rows = [_feature_payload(feature, index, source_dataset) for index, feature in enumerate(data["features"])]
