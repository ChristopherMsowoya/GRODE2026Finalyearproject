"""
ESRI Phase I — Full database schema setup script.

Creates ALL tables required by the PRD:
  - mw_boundary
  - admin_boundaries
  - grid_cells
  - grid_admin_intersections
  - daily_rainfall
  - seasonal_onset
  - false_onset_events
...
Run from project root:
    python -m backend.database.setup_schema
"""
...
CREATE TABLE IF NOT EXISTS admin_boundaries (
    ...
    admin_level         INTEGER NOT NULL,   -- 1=region, 2=district, 3=TA
...
CREATE INDEX IF NOT EXISTS idx_daily_rainfall_date
    ON daily_rainfall(rainfall_date);

-- ============================================================
-- 10. SEASONAL ONSET (per grid, per season year)
-- ============================================================
CREATE TABLE IF NOT EXISTS seasonal_onset (
    onset_id                    BIGSERIAL PRIMARY KEY,
    grid_id                     TEXT NOT NULL REFERENCES grid_cells(grid_id) ON DELETE CASCADE,
    season_year                 INTEGER NOT NULL,   -- year the rainy season starts (e.g. 2021 for 2021-22)
    onset_date                  DATE,
...
-- ============================================================
-- 11. FALSE ONSET EVENTS (raw, per grid, per season)
-- ============================================================
...
-- ============================================================
-- 12. FALSE ONSET PROBABILITY (climatological atlas)
-- ============================================================
...
-- ============================================================
-- 13. DRY SPELL EVENTS (raw, per grid, per season)
-- ============================================================
CREATE TABLE IF NOT EXISTS dry_spell_events (
    ...
    season_year                 INTEGER NOT NULL,
    onset_date                  DATE,
    dry_spell_threshold         INTEGER NOT NULL,  -- 5, 7, or 9 days
...
CREATE INDEX IF NOT EXISTS idx_dry_spell_events_threshold
    ON dry_spell_events(dry_spell_threshold);

-- ============================================================
-- 14. DRY SPELL PROBABILITY (climatological atlas)
-- ============================================================
CREATE TABLE IF NOT EXISTS dry_spell_probability (
    ...
    baseline_start          INTEGER NOT NULL,
    baseline_end            INTEGER NOT NULL,
    dry_spell_threshold     INTEGER NOT NULL,   -- 5, 7, or 9
...
-- ============================================================
-- 15. ONSET CLIMATOLOGY (long-term statistics per grid)
-- ============================================================
CREATE TABLE IF NOT EXISTS onset_climatology (
    ...
    median_onset_doy        INTEGER,   -- day-of-year
...
def setup_schema():
    ...
    with get_connection() as conn:
        ...
        conn.commit()
    print("✅ Schema setup complete — all tables created/verified.")
